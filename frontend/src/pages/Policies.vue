<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-2xl font-bold">Apólices</h2>
    ...
    <div v-if="showForm" class="bg-white rounded-xl shadow-sm p-6 mb-6">
      <h3 class="font-semibold mb-4">Nueva Apólice</h3>
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-xs text-gray-500 mb-1">Nombre del Asegurado</label>
        ...
        <div>
          <label class="block text-xs text-gray-500 mb-1">Tipo de Riesgo</label>
          <select v-model="form.risk_type" class="w-full border rounded-lg px-3 py-2 text-sm">
            <option value="low">Bajo (×1.0)</option>
            <option value="medium">Medio (×1.5)</option>
            <option value="high">Alto (×2.2)</option>
          </select>
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">Prima Base (€)</label>
          <input v-model.number="form.base_premium_eur" type="number" step="0.01"
      ...
      <div class="flex gap-2 mt-4">
        <button @click="createPolicy" class="bg-green-600 text-white px-4 py-2 rounded-lg text-sm">Crear</button>
        <button @click="showForm = false" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm">Cancelar</button>
      </div>
    </div>
      ...
      <div v-for="p in list" :key="p.id" class="bg-white rounded-xl shadow-sm p-5">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            <span :class="`text-xs px-3 py-1 rounded-full font-medium ${statusColor(p.status)}`">{{ p.status }}</span>
            <div class="text-right">
              <p class="font-bold">€{{ p.premium?.toFixed(2) }}</p>
              <p class="text-xs text-gray-400">Riesgo: {{ p.risk_type }}</p>
            </div>
          </div>
        </div>
        <div v-if="transitions[p.status]" class="mt-3 pt-3 border-t flex gap-2">
          <button v-for="s in transitions[p.status]" :key="s"
    ...
    <p v-if="list.length === 0" class="text-center py-12 text-gray-400">No hay apólices registradas</p>
  </div>
</template>
...
const form     = ref({ insured_name: '', insured_document: '', risk_type: 'low', base_premium_eur: 500, starts_at: '', expires_at: '' })
...
const load = async () => {
  try { list.value = (await policies.list()).data.data || [] } catch {}
}
