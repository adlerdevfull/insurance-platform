  ...
  <div>
    ...
    <div class="grid grid-cols-2 gap-6">
      <div class="bg-white rounded-xl shadow-sm p-6">
        <h3 class="font-semibold mb-4">Flujo de Apólices</h3>
      ...
      <div class="bg-white rounded-xl shadow-sm p-6">
        <h3 class="font-semibold mb-4">Flujo de Sinistros</h3>
        <div class="flex flex-wrap gap-2 text-sm">
...
onMounted(async () => {
  try { policyList.value = (await policies.list({ per_page: 100 })).data.data || [] } catch {}
  try { claimList.value  = (await claims.list({ per_page: 100 })).data.data || []  } catch {}
})

const byStatus = (list: any[], s: string) => list.filter(i => i.status === s).length

const cards = computed(() => [
  { label: 'Apólices Activas',   value: byStatus(policyList.value, 'active'),      icon: '📋', color: 'bg-blue-500' },
  { label: 'Sinistros Abiertos', value: byStatus(claimList.value, 'reported'),     icon: '⚠️',  color: 'bg-orange-500' },
  { label: 'En Revisión',        value: byStatus(claimList.value, 'under_review'), icon: '🔍', color: 'bg-yellow-500' },
  { label: 'Aprobados',          value: byStatus(claimList.value, 'approved'),     icon: '✅', color: 'bg-green-500' },
])
