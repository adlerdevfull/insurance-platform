<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-2xl font-bold">Sinistros</h2>
    ...
    <div v-if="showForm" class="bg-white rounded-xl shadow-sm p-6 mb-6">
      <h3 class="font-semibold mb-4">Reportar Sinistro</h3>
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-xs text-gray-500 mb-1">ID de Apólice</label>
          <input v-model.number="form.policy_id" type="number" class="w-full border rounded-lg px-3 py-2 text-sm" />
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">Importe Reclamado (€)</label>
          <input v-model.number="form.claimed_eur" type="number" step="0.01" class="w-full border rounded-lg px-3 py-2 text-sm" />
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">Fecha del Siniestro</label>
      ...
      <div class="flex gap-2 mt-4">
        <button @click="reportClaim" class="bg-orange-600 text-white px-4 py-2 rounded-lg text-sm">Reportar</button>
        <button @click="showForm = false; formError = null" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm">Cancelar</button>
      </div>
    </div>
      ...
      <div v-for="c in list" :key="c.id" class="bg-white rounded-xl shadow-sm p-5">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-500">Apólice #{{ c.policy_id }} · {{ c.occurred_at }}</p>
            <p class="text-xs text-gray-400 mt-1">{{ c.description }}</p>
          </div>
          <div class="flex items-center gap-3">
            <span :class="`text-xs px-3 py-1 rounded-full font-medium ${statusColor(c.status)}`">{{ c.status }}</span>
            <div class="text-right">
              <p class="font-bold">€{{ c.claimed_amount?.toFixed(2) }}</p>
              <p v-if="c.approved_amount" class="text-xs text-green-600">Aprobado: €{{ c.approved_amount?.toFixed(2) }}</p>
            </div>
          </div>
        </div>
        <div v-if="c.status === 'under_review'" class="mt-3 pt-3 border-t flex gap-2">
          <button @click="reviewClaim(c.id, 'approve', c.claimed_amount)"
            class="text-xs bg-green-50 text-green-700 px-3 py-1.5 rounded-lg hover:bg-green-100 transition">
            ✅ Aprobar
          </button>
          <button @click="reviewClaim(c.id, 'reject')"
        ...
        <div v-if="c.status === 'reported'" class="mt-3 pt-3 border-t">
          <button @click="startReview(c.id)"
            class="text-xs bg-yellow-50 text-yellow-700 px-3 py-1.5 rounded-lg hover:bg-yellow-100 transition">
            🔍 Iniciar Revisión
          </button>
        </div>
        <p v-if="c.rejection_reason" class="mt-2 text-xs text-red-500">Motivo: {{ c.rejection_reason }}</p>
      </div>
    </div>
    <p v-if="list.length === 0" class="text-center py-12 text-gray-400">No hay sinistros registrados</p>
  </div>
</template>
...
const load = async () => {
  try { list.value = (await claims.list()).data.data || [] } catch {}
}
...
const reportClaim = async () => {
  ...
  try {
    await claims.create({ ...form.value, claimed_amount_cents: Math.round(form.value.claimed_eur * 100) })
    showForm.value = false
    load()
  } catch (e: any) {
    formError.value = e.response?.data?.error ?? 'Erro ao reportar sinistro'
  }
}

const startReview = async (id: number) => {
  // transition reported → under_review via review endpoint com decision especial
  // usamos o endpoint de transição direto
...
const reviewClaim = async (id: number, decision: string, amount?: number) => {
  await claims.review(id, {
    ...
    rejection_reason: decision === 'reject' ? 'No cumple los requisitos de cobertura' : undefined,
  })
  load()
}
</script>
